...
const PickupVerificationScreen = () => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (trip.documents && trip.documents.length > 0 && pickupStop?.id) {
          ...
          if (serverCargoDocs.length > 0) {
            const serverPhotos: CapturedPhoto[] = serverCargoDocs.slice(0, 3).map((d: any) => {
    ...
    loadDraft();
  }, [trip?.id, isReturnLoading, pickupStop?.id, trip?.documents, markUploaded]);
  ...
  const acceptPhoto = (photo: CapturedPhoto) => {
    const valid = [...photosRef.current, photo].filter(Boolean).slice(0, 3);
    photosRef.current = valid;
    setPhotos(valid);
    savePhotoDrafts(valid);
    uploadPhotoNow(photo);
    if (valid.length >= 3) setCameraOpen(false);
  };
   ...
   */
  const addPhoto = async (source: 'camera' | 'gallery' = 'camera') => {
    if (photosRef.current.length >= 3) return;
    if (source === 'camera') {
      setCameraOpen(true);
      return;
    }
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F8FAFC' }}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.uploadSectionCard}>
          ...
          <View style={styles.photosGrid}>
            {[0, 1, 2].map((i) => (
              <TouchableOpacity
                ...
                onLongPress={photos[i] ? undefined : () => addPhoto('gallery')}
              >
                {photos[i] ? (
                  <>
          ...
          {!hasAllPhotos ? (
            <TouchableOpacity style={styles.galleryLink} activeOpacity={0.7} onPress={() => addPhoto('gallery')}>
              <Text style={styles.galleryLinkText}>{language === 'ur' ? 'گیلری سے منتخب کریں' : 'Choose from gallery'}</Text>
            </TouchableOpacity>
          ) : null}
