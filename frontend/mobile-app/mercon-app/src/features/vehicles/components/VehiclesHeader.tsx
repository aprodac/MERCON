...
/** Fixed (non-scrolling) page header: title + subtitle on the left, filter/menu icon buttons on the right. */
export function VehiclesHeader({ onFilterPress, onMenuPress, filterActive, className }: VehiclesHeaderProps) {
  return (
    <View className={`flex-row items-center justify-between bg-white px-4 pb-3 pt-3 border-b border-[#EEF1F6] ${className ?? ''}`}>
      <View className="gap-0.5">
        <Text className="text-[24px] font-extrabold leading-[28px] text-[#3E3C3D]">Vehicles</Text>
        <Text className="text-[12px] font-medium text-gray-500">Fleet vehicles & active assignments</Text>
      </View>
