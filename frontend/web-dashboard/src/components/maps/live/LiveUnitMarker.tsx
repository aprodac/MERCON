...
import { unitTitle } from '@/lib/fleetLive';
 ...
 *  - hollow dot, faded → offline (showing the last known fix)
 ...
 */
function LiveUnitMarkerImpl({ unit, selected, dimmed, onSelect }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        {selected && (
          <span
            className="absolute top-1/2 left-1/2 size-14 -translate-x-1/2 -translate-y-[calc(50%+8px)] rounded-full animate-ping opacity-30"
        ...
        >
          {moving ? (
            ...
            <span className="size-3 rounded-full border-[3px] bg-transparent" style={{ borderColor: '#94a3b8' }} />
          ) : (
        ...
        <span
          className={cn(
            'mt-1 hidden max-w-[120px] truncate rounded-md px-1.5 py-px font-mono text-[10px] font-semibold leading-4 shadow-sm ring-1 ring-black/5',
            'bg-white/95 text-slate-800 dark:bg-slate-900/90 dark:text-slate-100 dark:ring-white/10',
            'group-data-[labels=on]/map:block',
            (selected || offline) && 'block',
