...
export default function APAgeingPage() {
  ...
  const { data: reportRes, isLoading, isError, refetch } = useQuery({
    queryKey: ['finance-reports', 'ap-ageing', asOfDate],
    queryFn: () =>
  ...
  return (
    <DashboardLayout active="finance" title="AP Ageing Report">
      <div className="p-6 space-y-6 max-w-7xl mx-auto">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-[#3E3C3D]">Accounts Payable Ageing</h1>
            <p className="text-sm text-slate-500">Provider & vendor outstanding bills by overdue range</p>
          </div>
        ...
        {isError && (
          <div className="bg-rose-50 border border-rose-200 rounded-xl p-4 text-rose-800 text-sm">
            Failed to load AP Ageing report. Please try again.
          </div>
        )}
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="bg-slate-50/80 border-b border-slate-200/80 text-xs font-semibold text-slate-600 uppercase tracking-wider">
                    <th className="py-3 px-4">Provider / Payee Name</th>
                  ...
                  {rows.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="py-8 text-center text-slate-400 text-sm">
                        No outstanding provider bills found.
                      </td>
                    </tr>
                  ) : (
                    rows.map((row) => (
