...
function BankSparkline({ data, colorHex = '#FA634E' }: { data?: Array<{ date: string; balance: number }>; colorHex?: string }) {
  if (!data || data.length < 2) {
    return <div className="h-9 w-full bg-slate-100 dark:bg-slate-800/50 rounded-lg flex items-center justify-center text-[10px] text-slate-400">No chart data</div>;
  }
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <Link
                ...
                className="font-bold text-slate-900 dark:text-slate-100 hover:text-[#FA634E] dark:hover:text-[#FA634E] transition-colors text-xs block"
              >
                {row.is_cash ? 'Cash Drawer' : row.bank_name || 'Bank Account'}
              </Link>
              <span className="text-[11px] text-slate-400 font-mono">{row.is_cash ? 'Cash' : maskAccountNumber(row.account_number)}</span>
            </div>
          </div>
        );
      },
      ...
      accessor: (row) =>
        row.account ? (
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-slate-700 dark:text-slate-300 truncate max-w-[160px]">{row.account.name}</span>
          </div>
        ) : (
      ...
      accessor: (row) => (
        <div className="text-right">
          ...
          {Number(row.opening_balance || 0) !== 0 && (
            <span className="block text-[10px] text-slate-400">incl. {formatMoney(Number(row.opening_balance))} opening</span>
          )}
        </div>
      ),
    },
    ...
    <DashboardLayout active="finance" title="Bank Accounts">
      <div className="p-4 space-y-3.5 max-w-[1400px] mx-auto animate-fade-in">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200/80 dark:border-slate-800 pb-2.5">
          ...
          <div className="flex items-center gap-1.5 bg-[#F4F4F5] dark:bg-slate-900 p-1 rounded-xl">
            <button
            ...
            >
              ...
              <span className={`px-1.5 py-0.2 rounded-full text-[10px] font-extrabold ${categoryFilter === 'all' ? 'bg-slate-100 dark:bg-slate-700 text-slate-800 dark:text-slate-200' : 'bg-slate-200/70 dark:bg-slate-800 text-slate-600 dark:text-slate-400'}`}>
                {counts.all}
              </span>
            </button>
            ...
            >
              ...
              <span>Bank</span>
              <span className={`px-1.5 py-0.2 rounded-full text-[10px] font-extrabold ${categoryFilter === 'bank' ? 'bg-sky-100 text-sky-800' : 'bg-slate-200/70 dark:bg-slate-800 text-slate-600 dark:text-slate-400'}`}>
                {counts.bank}
              </span>
            </button>
            ...
            >
              ...
              <span>Cash</span>
              <span className={`px-1.5 py-0.2 rounded-full text-[10px] font-extrabold ${categoryFilter === 'cash' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-200/70 dark:bg-slate-800 text-slate-600 dark:text-slate-400'}`}>
                {counts.cash}
              </span>
            </button>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex items-center gap-1.5 px-3 py-1 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl h-8">
              <Switch id="show-inactive-switch" checked={showInactive} onCheckedChange={setShowInactive} className="scale-75" />
              <label htmlFor="show-inactive-switch" className="text-xs font-semibold text-slate-600 dark:text-slate-400 cursor-pointer select-none">
                Show inactive
              </label>
            </div>

            {/* Cards vs Table View Toggle */}
            <ToggleGroup value={[viewMode]} onValueChange={(v) => v && v[0] && setViewMode(v[0] as any)} className="bg-slate-100 dark:bg-slate-900 p-0.5 rounded-xl border border-slate-200/70 dark:border-slate-800 h-8">
              <ToggleGroupItem value="cards" aria-label="Cards view" className="h-7 w-7 p-0 rounded-lg data-[state=on]:bg-white dark:data-[state=on]:bg-slate-800 data-[state=on]:shadow-2xs">
                <LayoutGrid className="w-3.5 h-3.5 text-slate-700 dark:text-slate-300" />
              </ToggleGroupItem>
              <ToggleGroupItem value="table" aria-label="Table view" className="h-7 w-7 p-0 rounded-lg data-[state=on]:bg-white dark:data-[state=on]:bg-slate-800 data-[state=on]:shadow-2xs">
                <TableIcon className="w-3.5 h-3.5 text-slate-700 dark:text-slate-300" />
              </ToggleGroupItem>
            </ToggleGroup>
            ...
            <Button
              ...
              className="h-8 text-xs font-bold gap-1.5 bg-[#FA634E] hover:bg-[#EE553F] text-white rounded-xl shadow-2xs"
            >
        ...
        {filteredAccounts.length === 0 ? (
          <div className="bg-white dark:bg-slate-900 rounded-[20px] border border-slate-200/80 dark:border-slate-800 p-8">
            <FinanceEmptyState
            ...
            {filteredAccounts.map((acc) => {
              ...
              return (
                <div
                  ...
                  className={`rounded-[20px] bg-white dark:bg-slate-900 border ${
                    acc.isActive
                ...
                >
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-1.5">
                          <h3 className="font-extrabold text-sm text-slate-900 dark:text-slate-100 group-hover:text-[#FA634E] dark:group-hover:text-[#FA634E] transition-colors truncate max-w-[150px]">
                            {acc.is_cash ? 'Cash Drawer' : acc.bank_name || 'Bank Account'}
                          </h3>
                          {!acc.isActive && (
                            <Badge variant="outline" className="text-[9px] font-bold px-1.5 py-0 bg-slate-100 text-slate-500 border-slate-200">
                              Inactive
                            </Badge>
                          )}
                        </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Book Balance</span>
                      {openingBal !== 0 && (
                        <HoverCard>
                          ...
                          <HoverCardContent align="end" className="w-64 p-3 text-xs space-y-1">
                            <p className="font-bold text-slate-900 dark:text-slate-100">Opening Balance Note</p>
                    ...
                    <div className="text-2xl font-extrabold fin-num text-slate-900 dark:text-slate-100 tracking-tight">
                  ...
                  <div className="flex items-center justify-between text-xs font-semibold pt-1 border-t border-slate-100 dark:border-slate-800/80">
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800/80 text-xs">
                    <div>
                      {acc.last_reconciled_at ? (
                        <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-200/80 text-[10.5px] font-semibold gap-1">
                          <CheckCircle2 className="w-3 h-3 text-emerald-600" />
                          <span>Reconciled to {formatDate(acc.last_reconciled_at)}</span>
                        </Badge>
                      ) : (
                        <Badge variant="outline" className="bg-slate-50 text-slate-500 border-slate-200/80 text-[10.5px] font-semibold">
                          Never reconciled
                        </Badge>
                      )}
                    </div>
                    ...
                    <Link
                      ...
                      className="text-[11.5px] font-bold text-slate-600 dark:text-slate-400 hover:text-[#FA634E] dark:hover:text-[#FA634E] transition-colors"
                    >
          ...
          /* Table View */
          <div className="bg-white dark:bg-slate-900 rounded-[20px] border border-slate-200/80 dark:border-slate-800 p-4 shadow-xs">
            <DataTable
