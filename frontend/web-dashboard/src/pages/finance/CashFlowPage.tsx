      ...
      <div className="p-6 space-y-6 max-w-5xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500">Indirect cash flow statement (Operating, Investing, Financing)</p>
          </div>
        ...
        {report && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Opening Cash</p>
                <p className="text-xl font-bold font-mono text-slate-900 mt-1">SAR {formatMoney(report.opening_cash)}</p>
              </div>
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Net Change in Cash</p>
              ...
              <div className={`p-2.5 rounded-lg ${report.net_change_in_cash >= 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Closing Cash</p>
          ...
          {isLoading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider pb-2 border-b border-slate-200">
                  Cash Flow from Operating Activities
                </h3>
                <div className="space-y-2 text-sm pl-2">
                  <div className="flex justify-between font-medium text-slate-900 py-1">
                  ...
                  {report.operating.adjustments.map((adj) => (
                    <div key={adj.account_code} className="flex justify-between text-slate-600 text-xs py-0.5">
                      <span>
                        <span className="font-mono text-slate-400 mr-2">{adj.account_code}</span>
                        {adj.name}
                      </span>
                  ...
                  {report.operating.adjustments.length === 0 && (
                    <p className="text-xs text-slate-400 italic py-0.5">No operating adjustments recorded.</p>
                  )}
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2 text-sm pl-2">
                  {report.investing.items.map((item) => (
                    <div key={item.account_code} className="flex justify-between text-slate-600 text-xs py-0.5">
                      <span>
                        <span className="font-mono text-slate-400 mr-2">{item.account_code}</span>
                        {item.name}
                      </span>
                  ...
                  {report.investing.items.length === 0 && (
                    <p className="text-xs text-slate-400 italic py-0.5">No investing activities recorded.</p>
                  )}
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2 text-sm pl-2">
                  ...
                  {report.financing.items.length === 0 && (
                    <p className="text-xs text-slate-400 italic py-0.5">No financing activities recorded.</p>
                  )}
